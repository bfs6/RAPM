    ...
    for possession in possessions:
        ...
        for lineup_type in home_or_away:
            for player_team in lineups_with_team_names[lineup_type]:
                ...
                converted_player_name = common_utils.nba_com_player_name_to_bball_ref_player_name(player_name, team_code, year)
                stub_name = common_utils.player_to_stub_name(converted_player_name)
...
def calculate_rapm(year, X, Y, metric="rapm", season_type="regular_season"):
    ...
    for player, opm, dpm in rapm:
        ...
        data = {
            'player': common_utils.nba_com_player_name_to_bball_ref_player_name(player_name, team_code, year),
            'team': team_code,
            'o' + metric: opm,
            'd' + metric: dpm,
            metric: opm + dpm
    ...
    sorted_opm = db.players.find(
        filter = {"player_index.season": year_as_int, "{}_possessions".format(season_type): {"$gt": poss_threshold}},
