...
for(ssn in seasons_loop){
  ...
  meanDF <- data.frame(varID = row.names(mean), RAPM = mean)
  ...
  meanDF <- data.frame(rapm_type = do.call(rbind, strsplit(meanDF$varID, "_ID"))[,1],
                       ...
                       RAPM = meanDF$RAPM)
  meanDF <- filter(meanDF, rapm_type != "Constant")
  ...
  # meanDF$RAPM <- ((meanDF$poss_def/meanDF$poss_tot) * meanDF$D_RAPM) + ((meanDF$poss_off/meanDF$poss_tot) * meanDF$O_RAPM)
  meanDF$RAPM <- meanDF$O_RAPM + meanDF$D_RAPM
  meanDF <- arrange(meanDF, -RAPM)
  
  # apm_player_stats_filter <- filter(apm_player_stats, season_name == paste0("NBA_", ssn))
  # apm_player_stats_filter <- remove_zero_var_cols(apm_player_stats_filter)
  # apm_player_stats_filter <- setDT(apm_player_stats_filter)[, .SD[1,], by = c("person_id")]
  # apm_player_stats_filter <- subset(apm_player_stats_filterDT, select = c(person_id, oapm, oapm_error, dapm, dapm_error, apm, apm_error, possession))
  # 
  # RAPM_DF <- merge(meanDF, apm_player_stats_filter, by = "person_id", all.x = T)
  # rankVars <- c("RAPM", "apm")
  # RAPM_DF <- setDT(RAPM_DF)[, paste0(rankVars, "_rank") := lapply(.SD, function(x) frank(-x, ties.method = "min")), .SDcols = rankVars]
  # RAPM_DF <- arrange(RAPM_DF, -RAPM)
  # RAPM_DF$RankDiff <- abs(RAPM_DF$RAPM_rank - RAPM_DF$apm_rank)
